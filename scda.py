...
class LyotCoronagraph(object): # Lyot coronagraph base class
    _file_fields = { 'fileorg': ['work dir', 'ampl src dir','plot dir', 'TelAp dir', 'FPM dir', 'LS dir',
    ...
    _aperture_menu = { 'prim': ['hex1', 'hex2', 'hex3', 'hex4', 'key24', 'pie12', 'pie08', 'circ', 
                                ...
                                'luvoir2017novAp01', 'luvoir2017novAp01c',
                                'luvoir2017novAp02', 'luvoir2017novAp02c',
                                'luvoir2017novAp03', 'luvoir2017novAp03c',
                                'luvoir2017novAp04', 'luvoir2017novAp04c',
                                'luvoir2017novAp05', 'luvoir2017novAp05c',
                                'luvoir2017novAp07', 'luvoir2017novAp07c',
                                'luvoir2017novAp08', 'luvoir2017novAp08c',
                                'luvoir2017novAp09', 'luvoir2017novAp09c',
                                'luvoir2017novAp10', 'luvoir2017novAp10c'],
    ...
    _square2circ_ratio = {'luvoir2017novAp04': 0.977, 'luvoir2017novAp04c': 0.977, 
                          'luvoir2017novAp05': 0.982, 'luvoir2017novAp05c': 0.982, 
                          'luvoir2017novAp07': 0.977, 'luvoir2017novAp07c': 0.977}
    ...
    def __init__(self, verbose=False, **kwargs):
        ...
        if not os.path.exists(self.fileorg['plot dir']):
            os.mkdir(self.fileorg['plot dir'])
    ...
    def write_eval_products(self, pixscale_lamoD=0.25, star_diam_vec=None, Npts_star_diam=7, Nlam=None, 
        ...
        trans_curve = sky_trans[sky_trans.shape[0]//2,
                                sky_trans.shape[1]//2:sky_trans.shape[1]//2+seps.shape[0]]
        ...
        sep_list_fname = os.path.join(self.fileorg['eval subdir'], 'sep_list.fits')
        intens_curve_fname = os.path.join(self.fileorg['eval subdir'], 'intens_vs_sep_and_diam.fits')
        trans_curve_fname = os.path.join(self.fileorg['eval subdir'], 'trans_vs_sep.fits')
        ...
        sep_list_hdu = pyfits.PrimaryHDU(seps, header=header)
        sep_list_hdu.writeto(sep_list_fname, clobber=True)
        intens_curve_hdu = pyfits.PrimaryHDU(stellar_intens_curves*contrast_convert_fac, header=header)
        intens_curve_hdu.writeto(intens_curve_fname, clobber=True)
        trans_curve_hdu = pyfits.PrimaryHDU(trans_curve, header=header)
        trans_curve_hdu.writeto(trans_curve_fname, clobber=True)
        ...
        logging.info("Wrote angular separation list to {:s}".format(sep_list_fname))
        logging.info("Wrote intensity curve array (vs angular sep and star diam) to {:s}".format(intens_curve_fname))
        logging.info("Wrote transmission curve (vs angular sep) to {:s}".format(intens_curve_fname))
...
class AxisymAPLC(LyotCoronagraph): # 1-D axisymmetric APLC following Zimmerman et al. (2016)
    ...
    def get_metrics(self, fp2res=16, verbose=True): # for Axisym APLC class
        ...
        rhos = np.matrix(np.arange(Nrho) * 1./fp2res).T
        drho = 1./fp2res
        p7ap_ind = np.less_equal(np.array(rhos.T), 0.7)[0]

        psisqr_unocc = np.zeros((Nlam, Nrho))
        psisqr_telap = np.zeros((Nlam, Nrho))
        ...
        for wi, wr in enumerate(wrs):
            ...
            psisqr_unocc[wi] = np.power(np.absolute(Psi_unocc.T), 2)
            psisqr_telap[wi] = np.power(np.absolute(Psi_telap.T), 2)
            peak_intens_unocc[wi] = np.power(np.absolute(Psi_unocc_peak), 2)[0]
            peak_intens_telap[wi] = np.power(np.absolute(Psi_telap_peak), 2)[0]
        
        bandavg_psisqr_unocc = np.mean(psisqr_unocc, axis=0)
        bandavg_psisqr_telap = np.mean(psisqr_telap, axis=0)
        bandavg_peak_unocc = np.mean(peak_intens_unocc, axis=0)
        bandavg_peak_telap = np.mean(peak_intens_telap, axis=0)
        
        fwhm_ind_APLC = np.greater_equal(bandavg_psisqr_unocc, bandavg_peak_unocc/2)
        fwhm_ind_TelAp = np.greater_equal(bandavg_psisqr_telap, bandavg_peak_telap/2)

        fwhm_sum_APLC = 2*np.pi*drho*np.sum(bandavg_psisqr_unocc[fwhm_ind_APLC]*rhos[fwhm_ind_APLC])
        fwhm_sum_TelAp = 2*np.pi*drho*np.sum(bandavg_psisqr_telap[fwhm_ind_TelAp]*rhos[fwhm_ind_TelAp])

        p7ap_sum_APLC = 2*np.pi*drho*np.sum(bandavg_psisqr_unocc[p7ap_ind]*rhos[p7ap_ind])
        p7ap_sum_TelAp = 2*np.pi*drho*np.sum(bandavg_psisqr_telap[p7ap_ind]*rhos[p7ap_ind])

        inc_energy_telap = np.sum(2*np.pi*np.multiply(rs, np.power(TelAp,2))*dr)

        self.eval_metrics['inc energy'] = inc_energy_telap
        self.eval_metrics['tot thrupt'] = 2*np.pi*drho*np.sum(bandavg_psisqr_unocc*rhos) / inc_energy_telap
        ...
        self.eval_metrics['apod nb res ratio'] = np.sum(np.abs(Apod - np.round(Apod)))/np.sum(TelAp)
...
class NdiayeAPLC(LyotCoronagraph): # Image-constrained APLC following N'Diaye et al. (2015, 2016)
    ...
    def get_coords(self, fp2res=8, rho_out=None, Nlam=None): # for APLC
        ...
        # FPM
        dmx = self.design['FPM']['rad']/(D * M_fp1)
        dmy = dmx
        mxs = np.matrix(np.linspace(-M_fp1+0.5,M_fp1-0.5,2*M_fp1)*dmx)
        mys = mxs
    ...
    def get_yield_input_products(self, pixscale_lamoD=0.25, star_diam_vec=None, Npts_star_diam=7, Nlam=None,
        ...
        intens_rad_vs_star_diam = np.zeros((len(star_diam_vec), M_fp2))
...
class HalfplaneAPLC(NdiayeAPLC): # N'Diaye APLC subclass for the half-plane symmetry case
    ...
    def write_slurm_script(self, queue_spec='auto', account='s1649', email=None, arch=None, overwrite=False, verbose=True):
        ...
        if queue_spec is 'auto':
            if self.design['LS']['aligntol'] is None:
                time_est_hrs = int(np.ceil(2.7*(self.design['Pupil']['N']/125.)**2*(self.design['Image']['Nlam']/3.)**3))
            else:
                time_est_hrs = int(np.ceil(3*(self.design['Pupil']['N']/125.)**2*(self.design['Image']['Nlam']/3.)**3))
            if time_est_hrs > 12:
                set_queue = """
